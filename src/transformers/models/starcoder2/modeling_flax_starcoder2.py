...
_CHECKPOINT_FOR_DOC = "bigcode/starcoder2-15b"
_CONFIG_FOR_DOC = "Starcoder2Config"
_REAL_CHECKPOINT_FOR_DOC = "bigcode/starcoder2-15b"
STARCODER2_START_DOCSTRING =  r"""
    ...
    Parameters:
        config ([`~STARCODER2Config`]): Model configuration class with all the parameters of the model.
...
STARCODER2_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`numpy.ndarray` of shape `({0})`):
            ...
            Indices can be obtained using [`~STARCODER2ConfiTokenizer`]. See
...
class FlaxStarcoder2RotaryEmbedding(nn.Module):
...
class FlaxStarcoder2MLP(nn.module):
    config: Starcoder2Config
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.c_fc = nn.Dense(self.config.intermediate_size, bias=self.config.use_bias, dtype=self.dtype)
        self.c_proj = nn.Dense(self.config.hidden_size, bias=self.config.use_bias, dtype=self.dtype)
...
# Copied from transformers.models.mistral.modeling_flax_mistral.FlaxMistralAttention with Mistral->Starcoder2
class FlaxStarcoder2Attention(nn.Module):
    config: Starcoder2Config
    dtype: jnp.dtype = jnp.float32
    ...
    def setup(self):
        ...
        self.rotary_emb = FlaxStarcoder2RotaryEmbedding(config, dtype=self.dtype)
...
class FlaxStarcoder2DecoderLayer(nn.Module):
    config: Starcoder2Config
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.input_layernorm = nn.LayerNorm(epsilon=self.config.norm_epsilon, dtype=self.dtype)
        self.self_attn = FlaxStarcoder2Attention(self.config, dtype=self.dtype)
        self.post_attention_layernorm = nn.LayerNorm(epsilon=self.config.norm_epsilon, dtype=self.dtype)
        self.mlp = FlaxStarcoder2MLP(self.config, dtype=self.dtype)
...
class FlaxStarcoder2PreTrainedModel(FlaxPreTrainedModel):
    """
...
# Copied from transformers.models.llama.modeling_flax_llama.FlaxLlamaLayerCollection with Llama->Starcoder2
class FlaxStarcoder2LayerCollection(nn.Module):
    config: Starcoder2Config
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.blocks = [
            FlaxStarcoder2DecoderLayer(self.config, dtype=self.dtype, name=str(i))
...
class FlaxStarcoder2Module(nn.Module):
    config: Starcoder2Config
    dtype: jnp.dtype = jnp.float32
    ...
    def setup(self):
        ...
        self.layers = FlaxStarcoder2LayerCollection(self.config, dtype=self.dtype)
        self.norm = nn.LayerNorm(self.config, dtype=self.dtype)
...
class FlaxStarcoder2Model(FlaxStarcoder2PreTrainedModel):
    module_class = FlaxStarcoder2Module
...
# Copied from transformers.models.llama.modeling_flax_llama.FlaxLlamaForCausalLMModule with Llama->Starcoder2
class FlaxStarcoder2ForCausalLMModule(nn.Module):
    config: Starcoder2Config
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.model = FlaxStarcoder2Module(self.config, dtype=self.dtype)
...
@add_start_docstrings("""Starcoder2 Model with a `language modeling` head on top for CLM training. """, STARCODER2_START_DOCSTRING)
class FlaxStarcoder2ForCausalLM(FlaxStarcoder2PreTrainedModel):
    module_class = FlaxStarcoder2ForCausalLMModule
