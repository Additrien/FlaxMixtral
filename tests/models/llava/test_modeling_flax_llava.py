...
class FlaxLlavaForConditionalGenerationModelTest(FlaxModelTesterMixin, unittest.TestCase):
    ...
    def test_equivalence_pt_to_flax(self):
        ...
        # Load Flax model from PyTorch checkpoint
        # Ensure dtype matches PT model if necessary (e.g., jnp.float16 if PT uses float16)
        # Adjust dtype based on the checkpoint's precision and testing environment
        fx_model = FlaxLlavaForConditionalGeneration.from_pretrained(model_id, from_pt=True, dtype=jnp.float32) # Assuming float32 for now
